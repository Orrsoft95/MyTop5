...
-  **Greedy Deduplication** & **Name similarity** are utilized to ensure variety; anime directly related to a user's selected top 5 titles (as listed by MAL) *or* to any already-generated recommendation are excluded from being listed as a later recommendation. Additionally, anime sharing 50% name similarity with a selected or already-recommended title are excluded as well.
...
├── MODEL_CARD.md
├── Dockerfile.md					# Docker image creation file utilized by Hugging Face Space
├── requirements.txt				# External (pip) libraries required to run MyTop5	
├── start.sh						# Shell script which populates the confidential IDs needed to run MyTop5 & then runs it.
└── README.md
...
| App framework | Streamlit & Hugging Face Spaces|
